...
def _copy_graph_info(graph: networkx.DiGraph, tree: networkx.DiGraph) -> networkx.DiGraph:
    for n in tree.nodes:
        tree.nodes[n]['name'] = graph.nodes[n]['name']

    for s, t in tree.edges:
        tree.edges[s, t]['forward_name'] = graph.edges[s, t]['forward_name']
...
class RoSE:
    ...
    # match nodes 함수 고치기
    def _match_nodes(self, graph1: networkx.DiGraph, top1, graph2: networkx.DiGraph, top2):
        # Collect all node labels
        # Simple Weisfeiler-lehman iteration
        node_labels_1 = {n: labels[-1]
                         for n, labels in networkx.weisfeiler_lehman_subgraph_hashes(graph1,
                         edge_attr='forward_name', node_attr='name', iterations=self._n_iter, include_initial_labels=True).items()}
        node_labels_2 = {n: labels[-1]
                         for n, labels in networkx.weisfeiler_lehman_subgraph_hashes(graph2,
                         edge_attr='forward_name', node_attr='name', iterations=self._n_iter, include_initial_labels=True).items()}

        # top-down matching
        # tree1 = _copy_graph_info(graph1, networkx.bfs_tree(graph1, source=top1))
        # tree2 = _copy_graph_info(graph2, networkx.bfs_tree(graph2, source=top2))
        # if self._is_logging:
        #     self._debug(f'Tree (REF): {tree1.edges()}')
        #     self._debug(f'Tree (HYP): {tree2.edges()}')
        #
        # node_hashes_1 = networkx.weisfeiler_lehman_subgraph_hashes(tree1, edge_attr='forward_name', node_attr='name',
        #                                                            iterations=self._n_iter, include_initial_labels=True)
        # node_hashes_2 = networkx.weisfeiler_lehman_subgraph_hashes(tree2, edge_attr='forward_name', node_attr='name',
        #                                                            iterations=self._n_iter, include_initial_labels=True)
        # if self._is_logging:
        #     self._debug(f'Hash (REF): {node_hashes_1}')
        #     self._debug(f'Hash (HYP): {node_hashes_2}')
        #
        # node_labels_1 = {}
        # node_labels_2 = {}
        # for depth in range(2):
        #     node_hash_d_1 = {n: labels[-(1 + depth)] + '_' + str(depth) for n, labels in node_hashes_1.items()
        #                      if n not in node_labels_1}
        #     node_hash_d_2 = {n: labels[-(1 + depth)] + '_' + str(depth) for n, labels in node_hashes_2.items()
        #                      if n not in node_labels_2}
        #
        #     # Find common
        #     common_labels = set(node_hash_d_1.values()).intersection(node_hash_d_2.values())
        #     node_labels_1.update({n: l for n, l in node_hash_d_1.items() if l in common_labels})
        #     node_labels_2.update({n: l for n, l in node_hash_d_2.items() if l in common_labels})
        # end
    ...
    def _relabeling_node(self, g: penman.Graph):
        try:
            ...
            return standardizer_rose.to_standard_amr(g)
        except Exception as e:
            if self._is_logging:
                self._debug(f'Reification/Relabeling failed for the following graph: {encode(g)}', exc_info=e)
            raise e

    def _get_verbose_graph(self, graph: penman.Graph) -> Tuple[Optional[networkx.DiGraph], str]:
        ...
        for identifier in sorted(graph.instances(), key=lambda t: t.target or ''):
            if identifier.source is None:
                continue
            new_graph.add_node(identifier.source, name=anyascii.anyascii(identifier.target) or '', is_variable=True)
        ...
        for edge in sorted(graph.attributes(), key=lambda t: (t.source, t.role, t.target)):
            ...
            new_graph.add_node(attr_value, name=anyascii.anyascii(target), is_variable=False)
    ...
    def compute_graph_similarity(self, reference: penman.Graph, hypothesis: penman.Graph) -> float:
        reference, ref_top = self._get_verbose_graph(reference)
        hypothesis, hyp_top = self._get_verbose_graph(hypothesis)
        maximum_score = 0.0
        for reference, hypothesis, matched_names in self._match_nodes(reference, ref_top, hypothesis, hyp_top):
            if self._is_logging:
                self._debug(f'Number of matched nodes: {len(matched_names)}')
            reference_edges = {(s, r, t)
                               for s, t, roles in reference.edges(data='forward')
                               for r in roles}
            reference_edges.update({(n, '/', d['name'])
                                    for n, d in reference.nodes(data=True) if d['is_variable']})
            hypothesis_edges = {(s, r, t)
                                for s, t, roles in hypothesis.edges(data='forward')
                                for r in roles}
    ...
    def compute(self, references: List[penman.Graph], hypotheses: List[penman.Graph], return_all_scores: bool = False):
        ...
        assert isinstance(references[0], penman.Graph), 'Reference should be a list of penman.Graphs!'
        assert isinstance(hypotheses[0], penman.Graph), 'Hypotheses should be a list of penman.Graphs!'

        scores = []
        for ref, hyp in zip(references, hypotheses):
            ref_id = ref.metadata.get("id", "NO_ID")
            hyp_id = hyp.metadata.get("id", "NO_ID")
    ...
    def compute_from_files(self, ref_files: List[str], hyp_files: List[str],
                           model: penman.model.Model = None, encoding: str = None,
