...
from networkx.algorithms import weisfeiler_lehman_subgraph_hashes
from .mcsplit import mcsplit
...
class RoSE:
    ...
    def __init__(self, ignore_wiki_role: bool = True, verbosity=None, num_iterations: int = 1,
    ...
    def _match_nodes(self, graph1: networkx.DiGraph, graph2: networkx.DiGraph):
        ...
        graph1 = networkx.relabel_nodes(graph1, node_labels_1)
        graph2 = networkx.relabel_nodes(graph2, node_labels_2)
        if self._is_logging:
            self._debug(f'Graph1\'s nodes: {graph1.nodes}')
            self._debug(f'Graph2\'s nodes: {graph2.nodes}')
        matching = list(mcsplit(graph1, graph2))
    ...
    def _get_verbose_graph(self, graph: penman.Graph) -> Optional[networkx.DiGraph]:
        ...
        for identifier in sorted(graph.instances(), key=lambda t: t.target or ''):
            if identifier.source is None:
                continue
            new_graph.add_node(identifier.source, name=unidecode(identifier.target) or '', is_variable=True)
        ...
        for edge in sorted(graph.attributes(), key=lambda t: (t.source, t.role, t.target)):
            ...
            new_graph.add_node(attr_value, name=unidecode(target), is_variable=False)
        ...
        for src, tgt in sorted(edges.keys()):
            ...
            new_graph.add_edge(src, tgt,
                               ...
                               strname=unidecode(','.join(sorted(forward_roles.union({r + '-of' for r in backward_roles})))),
                               forward=forward_roles,
                               connections=connections)
            new_graph.add_edge(tgt, src,
                               name=sorted(backward_roles.union({r + '-of' for r in forward_roles})),
                               strname=unidecode(','.join(sorted(backward_roles.union({r + '-of' for r in forward_roles})))),
    ...
    def compute_graph_similarity(self, reference: penman.Graph, hypothesis: penman.Graph) -> float:
        reference = self._get_verbose_graph(reference)
        hypothesis = self._get_verbose_graph(hypothesis)
        maximum_score = 0.0
        for reference, hypothesis, matched_names in self._match_nodes(reference, hypothesis):
            ...
            pr_ref = networkx.pagerank(reference)
            pr_hyp = networkx.pagerank(hypothesis)
            ...
            truth = sum(pr_ref[s] / reference.out_degree(s) for s, _ in reference_edges)
            positive = sum(pr_hyp[s] / hypothesis.out_degree(s) for s, _ in hypothesis_edges)
            true_positive_ref = sum(pr_ref[s] / reference.out_degree(s) for s, _ in edge_true_positive)
            true_positive_hyp = sum(pr_hyp[s] / hypothesis.out_degree(s) for s, _ in edge_true_positive)

            pr_recall = true_positive_ref / truth if truth else float(true_positive_ref == 0)
            pr_precision = true_positive_hyp / positive if positive else float(true_positive_hyp == 0)
